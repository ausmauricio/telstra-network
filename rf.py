...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun 25 19:05:22 2018
...
#loc_idxs_train = train.location.value_counts().head(485).index
...
log_vol = log_feature[log_feature.volume.isin(vol_idxs)].copy()
#log_vol = log_vol.drop(columns = "log_feature")
log_vol = pd.get_dummies(log_vol, prefix = "log_vol_feature", columns = ["volume",],  dummy_na=False)
log_vol = log_vol.groupby("id").agg(lambda x: sum(list(x)))
log_vol = log_vol[log_vol.index.isin(train.index)]
"""
...
def logloss(x,y,clf):
    ...
    y = np.array([y_dict[y_val] for y_val in y])
...
for depth in range(2,3):
    ...
    for it in range(15,17):
        ...
        clf = RandomForestClassifier(n_estimators=500, 
                                     ...
                                     #oob_score=True,
                                     #warm_start=True
                                     #class_weight = {0:0.65,1:0.25,2:0.1}
                                     ...
                                     #, random_state=200,#it*2,
                                     #max_features = None,
                                     #n_estimators = 60, warm_start = False,
                                     #class_weight = {0:0.65,1:0.25,2:0.1})
        ...
        print("training error: ", 1 - clf.score(X_train, Y_train))#, sample_weight = w))
        print("validation error: ", 1 - clf.score(X_test, Y_test))#, sample_weight = w))
        #print("validation error: ", w.dot(clf.predict(X_test)!=Y_test))
        ...
        coef = (Y_train == pred)
        coef = coef + np.zeros(N)
        coef[coef==0]=-1
        
        w = w * np.exp(- alpha * coef)
        w = w / w.sum()
        """
        
        print("log loss 1", log_loss(Y_train, clf.predict_proba(X_train)))
        print("log loss 2", log_loss(Y_test,  clf.predict_proba(X_test)))
        #print("log loss 1", logloss(X_train, Y_train,clf))
        #print("log loss 2", logloss(X_test, Y_test,clf))
